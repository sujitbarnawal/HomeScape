...
router.route('/')
  .get(wrapAsync(index))
  .post(isLoggedIn, validateListing,upload.array("listing[images]"), wrapAsync(listingController.createNew))
...
router.route('/:id')
  .put(isLoggedIn,isOwner,upload.array("listing[images]"), validateListing, wrapAsync(listingController.update))
